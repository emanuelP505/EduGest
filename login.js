...
async function fazerLogin(e) {
  ...
  try {
    ...
    // 4. CHECA SE ESCOLA TÁ CONGELADA
    const escolaDoc = await getDoc(doc(db, 'escolas', dados.escolaId));
    if (escolaDoc.exists() && escolaDoc.data().ativo === false) {
      await signOut(auth);
      window.location.href = 'bloqueado.html';
      return;
    }

    // 5. CHECA SE APP TÁ EM MANUTENÇÃO
    const configDoc = await getDoc(doc(db, 'config', 'app'));
    if (configDoc.exists() && configDoc.data().manutencao === true && dados.perfil !== 'admin') {
      await signOut(auth);
