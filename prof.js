...
let unsubProf, unsubTurmas, unsubEscola;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  auth.onAuthStateChanged(async (firebaseUser) => {
    // SE TÁ OFFLINE, USA CACHE DO FIRESTORE
    if (!navigator.onLine) {
...
function limparFormulario() {
  ...
  document.getElementById('biProf').value = '';
...
async function gerarPdfFolha() {
  ...
  doc.autoTable({
    startY: 35,
    head: [['Nº', 'Nome', 'Disciplina', 'Contrato', 'Salário (KZ)']],
